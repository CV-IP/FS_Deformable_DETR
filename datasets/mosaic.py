...
class MosaicDetection(Dataset):
    ...
    def __init__(
        self, dataset, img_size=(640, 640), mosaic=True, preproc=None,
        degrees=10.0, translate=0.1, mosaic_scale=(0.5, 1.5),
        mixup_scale=(0.5, 1.5), shear=2.0, perspective=0.0,
        enable_mixup=True, mosaic_prob=1.0, mixup_prob=0.5, *args
    ):
        """
        Args:
    ...
    def __getitem__(self, idx):
        ...
        # get_size_with_aspect_ratio(image_size, size, max_size=None)
        ...
        if self.enable_mosaic and random.random() < self.mosaic_prob:
            ...
            mosaic_img, mosaic_labels, iscrowds = random_perspective(
                mosaic_img,
                mosaic_labels,
    ...
    def mixup(self, origin_img, origin_labels, input_dim, iscrowds = None):
        jit_factor = random.uniform(*self.mixup_scale)
        FLIP = random.uniform(0, 1) > 0.5
        cp_labels = []
        while len(cp_labels) == 0:
            cp_index = random.randint(0, self.__len__() - 1)
            img, target = self._dataset.__getitem__(cp_index)
            img = np.asarray(img)
            bboxes = target['boxes']
            classes = target['labels']
